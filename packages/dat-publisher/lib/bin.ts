...
program
  ...
  .option('--del', 'Delete files that are removed from source directory')
  .option(
    '--seedtime <seedtime>',
    'seconds to seed for after updating',
    (val) => parseInt(val, 10),
    300,
  )
  .option(
    '--loadtimeout <timeout>',
    'timeout (in seconds) when loading the current version of the dat',
    (val) => parseInt(val, 10),
    60,
  )
  .action(async (address, pubdir, cmd) => {
    try {
      const { datDir, secret, verbose, seedtime, loadtimeout, del } = cmd;
